...
class ReviewModel(PropertyServiceItem):
    def __init__(self, itemId, **details):
        ...
        self.tenancyStart = details["tenancyStart"]
        self.tenancyEnd = details["tenancyEnd"]
    ...
    def validate_review_tenancy_dates(self):
        try:
            ...
            ts_date = datetime.strptime(self.tenancyStart, "%m-%Y").date

            # If tenancy is still active, validate start date is in past
            if self.tenancyEnd.upper() == "PRESENT":
                return date.today() > ts_date()

            te_date = datetime.strptime(self.tenancyEnd, "%m-%Y").date
